...
from cfg import visual_fontsize
...
def cvtImg2Bin(img_rgb, **kwargs):
    ...
    for key, value in kwargs.items():
        if key == 'verbose' and value == True:
            ...
            axes[0, 0].set_title('Color Thersholds', fontsize = visual_fontsize)
            axes[0, 0].imshow(color_binary, cmap = 'gray')

            axes[0, 1].set_title('Sobel Thersholds', fontsize = visual_fontsize)
            axes[0, 1].imshow(sxbinary, cmap = 'gray')

            axes[1, 0].set_title('Stacked Thresholds', fontsize = visual_fontsize)
            axes[1, 0].imshow(color_binary)

            axes[1, 1].set_title('Combined Color and Gradient Thresholds', fontsize = visual_fontsize)
